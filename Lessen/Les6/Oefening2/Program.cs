...
namespace Opdracht5
{
    ...
    {static void SchrijfLog(object output) => Console.WriteLine(output.ToString());
            ...
            static string GenerateAccount(string type, string vnaam, string fnaam){
                string code =""; 
                if (type.ToLower() == "docent")
                {
                    string kvnaam = GetStartOfString(vnaam, 4);
                    string kfnaam = GetStartOfString(fnaam, 2);
                    code = kvnaam.ToLower() + kfnaam.ToLower() + "@arteveldehs.be";
                }
                else if(type.ToLower() == "student"){
                    string kvnaam = GetStartOfString(vnaam, 4);
                    string kfnaam = GetStartOfString(fnaam, 4);
                    code = kvnaam.ToLower() + kfnaam.ToLower() + "@student.arteveldehs.be";
                } else
                {
